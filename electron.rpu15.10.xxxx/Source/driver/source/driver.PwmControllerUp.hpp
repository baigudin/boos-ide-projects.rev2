/** 
 * TI TMS320F2833x DSP Enhanced Pulse Width Modulator up counting controller.
 * 
 * @author    Sergey Baigudin, sergey@baigudin.software
 */
#ifndef DRIVER_PWM_CONTROLLER_UP_HPP_
#define DRIVER_PWM_CONTROLLER_UP_HPP_

#include "driver.PwmController.hpp"

class PwmControllerUp : public ::PwmController
{
...
public:
   ...
   */   
  PwmControllerUp(int32 clock, int32 index) : Parent(clock, index)
  {
  ...
  virtual float32 getDuty(int32 channel, bool set) const  
  {
    ...
    else
    {
      ...
      float32 cmp = static_cast<float32>(regPwm_->cmp[channel].val) + 1.0f;
      enable(is);
  ...
  virtual bool enableHighResolution()
  {
    ...
    regPwm_->hrcnfg.bit.edgmode = 0x2;
...
private:
  ...
  bool construct()
  {
    ...
    regPwm_->tbctl.bit.ctrmode = Tbctl::COUNT_UP;    
    for(int32 i=0; i<CHANNELS_NUMBER; i++)
    {
      ...
      switch(i)
      {
        case 0:
        {
          regPwm_->aqctl[i].bit.zro = Aqctl::SET;
          regPwm_->aqctl[i].bit.cau = Aqctl::CLEAR;
        }
        break;
        case 1:
        {
          regPwm_->aqctl[i].bit.zro = Aqctl::SET;
          regPwm_->aqctl[i].bit.cbu = Aqctl::CLEAR;
        }
        break;
        default: break;
      }
   ...
   */
  PwmControllerUp(const PwmControllerUp& obj);
   ...
   */
  PwmControllerUp& operator =(const PwmControllerUp& obj);    

};
#endif // DRIVER_PWM_CONTROLLER_UP_HPP_
